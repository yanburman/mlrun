...
async def post_event(
    ...
    logger.debug(f"Got event {name}", project=project)

    if not mlrun.api.crud.Events().is_valid_event(project, event_data):
        raise HTTPException(status_code=HTTPStatus.BAD_REQUEST.value)

    await run_in_threadpool(
        mlrun.api.crud.Events().process_event, db_session, event_data, name, project
    )
