...
async def store_alert_template(
    ...
    await (
        server.api.utils.auth.verifier.AuthVerifier().query_global_resource_permissions(
            mlrun.common.schemas.AuthorizationResourceTypes.alert_template,
    ...
    return await run_in_threadpool(
        server.api.crud.AlertTemplate().store_alert_template,
        db_session,
        name,
        alert_data,
    )
...
async def get_alert_template(
    ...
    return await run_in_threadpool(
        server.api.crud.AlertTemplate().get_alert_template, db_session, name
    )
...
async def list_alert_templates(
    ...
    return await run_in_threadpool(
        server.api.crud.AlertTemplate().list_alert_templates, db_session
    )
...
async def delete_alert_template(
    ...
    await run_in_threadpool(
        server.api.crud.AlertTemplate().delete_alert_template, db_session, name
    )
